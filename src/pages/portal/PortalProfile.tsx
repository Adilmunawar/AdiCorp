...
function DetailRow({ icon: Icon, label, value, editable, locked, onEdit, className = "" }: { icon: any, label: string, value: string, editable?: boolean, locked?: boolean, onEdit?: () => void, className?: string }) {
  ...
  return (
    <div className="flex items-center justify-between p-2.5 px-3 sm:px-4 bg-transparent hover:bg-muted/40 transition-colors active:bg-muted/60 gap-2">
      <div className="flex items-center gap-2.5 shrink-0">
        <div className="w-7 h-7 rounded-lg bg-primary/5 flex items-center justify-center shrink-0 shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)] border border-primary/5">
          <Icon className="w-3.5 h-3.5 text-primary/80" strokeWidth={2.5} />
        </div>
        <span className="text-[9px] font-bold text-muted-foreground/80 uppercase tracking-widest">{label}</span>
      </div>
      <div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
        <span className={`text-[11px] font-semibold text-foreground text-right tracking-wide truncate ${className}`}>{value}</span>
        <div className="w-6 h-6 flex items-center justify-center shrink-0">
          {editable && (
            <button onClick={onEdit} className="p-1.5 bg-primary/5 hover:bg-primary/15 rounded-md text-primary transition-all active:scale-90" title="Request Edit">
